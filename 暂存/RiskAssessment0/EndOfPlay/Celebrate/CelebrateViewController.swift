...
class CelebrateViewController: UIViewController {
   ...
   //     modalTransitionStyle = .crossDissolve
        ...
        dismissButton.frame = CGRect(x: mainFrame.maxX - 44, y: 22, width: 34, height: 34)
...
//        backView.removeFromSuperview()
    ...
    func showCelebrate() {
        ...
        // frame infos
        let hMargin = 10 * mainFrame.width / 355
        let vMargin = 15 * mainFrame.height / 544
        
        let mainLength = mainFrame.width - 2 * hMargin
        let titleHeight = mainFrame.height * 0.06
        // title
        ...
        // image
        let picHeight = mainFrame.height * 0.4
        ...
        // button frames
        let remainHeight = mainFrame.height - desLabel.frame.maxY - vMargin * 2
        let gap = 0.05 * remainHeight
        let buttonWidth = mainLength * 0.75
        let buttonHeight = (remainHeight - 2.5 * gap) * 0.25
        let buttonX = (mainFrame.width - buttonWidth) * 0.5
    ...
    func continueNextGame()  {
        ...
        holdAndPush(ABookLandingPageViewController())
    }
