...
class IndividualAnswerCell: UITableViewCell {
    ...
    class func cellWithTableView(_ tableView: IndividualAnswerTableView, row: Int, text: String?, image: URL?, options:[String], answerIndex: Int, withDetail: Bool) -> IndividualAnswerCell {
        ...
        cell!.leftImageView.sd_setShowActivityIndicatorView(true)
        cell!.leftImageView.sd_setIndicatorStyle(.gray)
        
        cell!.leftImageView.sd_setImage(with: image, placeholderImage: ProjectImages.sharedImage.placeHolder, options: .refreshCached)
        let isOdd = (row % 2 == 0)
    ...
    fileprivate func addSubs() {
        ...
        answersView = AnswersCollectionView.createAnswers(CGRect(x: 0, y: 0, width: 300, height: 30), options: ["ab","cd"], answerIndex: -1)
