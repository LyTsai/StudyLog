...
class VisualMapOptionCell: UICollectionViewCell {
    ...
    override func layoutSubviews() {
        ...
        textLabel.font = UIFont.systemFont(ofSize: 9 * fontFactor, weight: UIFont.Weight.bold)
...
class VisualMapOptionsView: UICollectionView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        var chosen = false
        if let selection = card.currentSelection() {
            chosen = (selection == indexPath.item)
        }
        
        var text = match?.statement ?? ""
        var imageURL = match?.imageUrl
        if card.isJudgementCard() {
            text = indexPath.item == 0 ? "ME: \(text)" : "NOT ME: \(text)"
            if imageURL == nil {
               imageURL = card.cardOptions.first?.match?.imageUrl
            }
        }
        
        cell.configureCellWithText(text, imageUrl: imageURL, chosen: chosen, color: match?.classification?.realColor ?? tabTintGreen)
