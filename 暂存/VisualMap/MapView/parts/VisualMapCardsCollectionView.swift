...
class VisualMapCardCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        titleLabel.font = UIFont.systemFont(ofSize: 12 * fontFactor, weight: UIFont.Weight.medium)
...
// collection view
class VisualMapCardsCollectionView: UICollectionView, UICollectionViewDataSource, UICollectionViewDelegate {
    var cards = [String]() {
        didSet{
            reloadData()
        }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let card = collection.getCard(cards[indexPath.item])!
        // card change view
        let changeVC = SingleCardViewController()
        changeVC.card = card
