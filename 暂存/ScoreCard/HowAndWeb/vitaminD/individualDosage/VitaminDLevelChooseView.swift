...
class VitaminDLevelChooseView: UIView {
    ...
    override func layoutSubviews() {
        ...
        let textRect = CGRect(x: radius * 2 + bounds.height * 0.5 , y: 0, width: bounds.width - 2 * radius - 2 * bounds.height, height: bounds.height).insetBy(dx: lineWidth, dy: lineWidth)
        label.frame = textRect
        label.font = UIFont.systemFont(ofSize: bounds.height * 0.28, weight: UIFont.Weight.medium)
    ...
    override func draw(_ rect: CGRect) {
        let radius = bounds.midY * 25 / 35
        let ballCenter = CGPoint(x: radius + bounds.height * 0.5, y: bounds.midY)
        let lineWidth = bounds.height / 35
...
//        ctx?.restoreGState()
        ...
        if abs(1 - blankRatio) < 1e-6 {
            drawString(NSAttributedString(string: "?", attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: bounds.height * 0.5, weight: UIFont.Weight.medium)]), inRect: CGRect(center: ballCenter, length: 2 * radius))
        }
    }
}
