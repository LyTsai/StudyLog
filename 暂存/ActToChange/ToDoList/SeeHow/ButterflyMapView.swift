...
class ButterflyMapView: VisualMapCustomView {
    ...
    fileprivate func setupSeeForUI() {
        ...
        avatar.frame = CGRect(center: CGPoint(x: bounds.midX, y: bounds.midY - standP * 6.5), width: 111 * standP, height: 76 * standP)
    ...
    fileprivate var cardTimer: Timer!
    ...
    fileprivate func resetCardNodes() {
        ...
        // reset
        let allCards = MatchedCardsDisplayModel.getAllMatchedCardsOfRiskClass(chosenRiskClassKey, riskTypeKey: chosenRiskTypeKey, forUser:userKey)
...
extension ButterflyMapView {
    ...
    fileprivate func autoPlayCards() {
        ...
        cardTimer = Timer.scheduledTimer(withTimeInterval: 1.5, repeats: true, block: { (timer) in
    ...
    fileprivate func drawCardsPlate() {
        ...
        let numberAttributes = [NSForegroundColorAttributeName: UIColor.black, NSFontAttributeName: UIFont.systemFont(ofSize: 14 * standP, weight: UIFontWeightMedium)]
