...
class CalendarCenter {
    class func getMondayStartCalendarOfDate(_ date: Date) -> (monthDays: [Int] , weekDays: [Int], dayIndex: Int) {
        ...
        var weekDays = [Int]()
        // MARK: ------ bug??
        for i in rowIndex * 7 ..< min((rowIndex + 1) * 7, totalNumber) {
    ...
    // get year
    class func getYearOfDate(_ date: Date) -> Int {
    ...
    // get the age
    class func getAgeFromDateOfBirth(_ birthday: Date!) -> Int {
        if birthday == nil {
            return 0
        }
        ...
        var age = currentComponents.year! - birthdayComponents.year!
    ...
    class func getAgeFromDateOfBirthString(_ birthday: String!) -> Int {
        return getAgeFromDateOfBirth(getDateFromString(birthday))
    }
    ...
    class func getDateFromString(_ dateString: String!) -> Date! {
        ...
        return ISO8601DateFormatter().date(from: dateString + "Z")
    }
