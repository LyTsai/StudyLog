...
class SingleCardView: JudgementCardTemplateView {
    ...
    // methods
    class func createWithFrame(_ frame: CGRect, image: UIImage?, text: String?, detail: String?, side: String?) -> SingleCardView {
        let singleCard = SingleCardView(frame: frame)
        singleCard.descriptionView.title = text ?? "Question"
        singleCard.descriptionView.detail = detail ?? "Description"
...
//        singleCard.backImages = backImages
    ...
    func denyAnswer() {
        ...
        // change backImage
        backImageView.image = normalBackImage
    ...
    func acceptAnswer() {
        ...
        // change badkImage
        backImageView.image = selectedBackImage
    ...
    fileprivate func reloadCards() {
        ...
        if options.isEmpty {
            ...
            for i in 0..<numberOfOptions {
                ...
                let cardView = SingleCardView()
                    
//                    .createWithFrame(cardFrame, image: option.match?.imageObj, text: vCard.metric?.name, prompt: option.match?.statement, side: option.match?.info)
    ...
    fileprivate func demoToShow() {
        for i in 0..<3 {
            let cardView = SingleCardView.createWithFrame(cardFrame, image: UIImage(named: "Coming Soon.png"), text: "Set of Cards", detail: "card statement", side: "the label")
            cardView.cardIndex = i + 1
            cardView.transform = (i == 0 ? CGAffineTransform.identity : backTransform)
            cardView.isUserInteractionEnabled = (i == 0)
    ...
    var backTransform: CGAffineTransform {
        ...
        while randomAngle == 0 {
            randomAngle = CGFloat(Double.pi) / 40 * (CGFloat(arc4random() % 7) - 3)
        }
