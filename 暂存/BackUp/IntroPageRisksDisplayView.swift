...
class IntroPageRisksDisplayView: UIView {
    ...
    fileprivate func addAndSetupWithNumber(_ number: Int) {
        ...
        // topLine
        let topLine = UIView(frame: CGRect(x: 0 ,y: 0, width: bounds.width, height: 1))
        topLine.backgroundColor = UIColor.white
        
        // back
        let path = UIBezierPath(roundedRect: bounds, byRoundingCorners: [.bottomLeft, .bottomRight], cornerRadii: CGSize(width: 8, height: 5)).cgPath
        backShape.path = path
        backShape.fillColor = fillColor
        // shadow
        ...
        backShape.shadowRadius = 2
        backShape.shadowOffset = CGSize(width: 0, height: 2)
