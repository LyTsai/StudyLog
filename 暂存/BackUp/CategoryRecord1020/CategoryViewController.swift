...
//        cartView.isHidden = true
    ...
    fileprivate func updateUI() {
        ...
        let buttons = buttonsView.addButtonsWithTitles(["Review"], buttonWRatio: 0.6, gap: 8, edgeInsets: UIEdgeInsets(top: 8, left: 25, bottom: 8, right: 25))
...
//        buttons.last!.addTarget(self, action: #selector(saveTouched), for: .touchUpInside)
    ...
    func showAllCards() {
        ...
        let allCards = cardCollection.getSortedCardsForRiskKey(focusingKey!)
        VDeckOfCardsFactory.metricDeckOfCards.attachToRiskFactorMetricCards(focusingKey!, metricsCards: allCards)
        
        let riskAssess = ABookRiskAssessmentViewController()
        navigationController?.pushViewController(riskAssess, animated: true)
    }
    
    func showCategoryCards() {
        let focusingKey = dataCursor.focusingRiskKey!
        let focusingCategory = dataCursor.focusingCategoryKey!
        let categoryCards = cardCollection.getOrganizedCardsOfRiskKey(focusingKey)[focusingCategory]!
        VDeckOfCardsFactory.metricDeckOfCards.attachToRiskFactorMetricCards(focusingKey, metricsCards: categoryCards)
