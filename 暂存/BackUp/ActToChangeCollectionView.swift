...
class PanelInfoCell: UICollectionViewCell {
    ...
    override func layoutSubviews() {
        ...
        let path = UIBezierPath(arcCenter: imageCenter, radius: radius, startAngle: 0, endAngle: CGFloat(Double.pi), clockwise: false)
        path.addLine(to: CGPoint(x: imageView.frame.minX ,y: imageView.frame.midY))
        let innerArc = UIBezierPath(arcCenter: imageCenter, radius: imageView.frame.width / 2, startAngle: CGFloat(Double.pi), endAngle: 0, clockwise: true)
