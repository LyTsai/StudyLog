...
class ScorecardResultBar: UIView {
    ...
    var scoreClassificationName = "Not Given"
    ...
    override func draw(_ rect: CGRect) {
        ...
        if drawScore {
            let mainRect = bounds.insetBy(dx: bounds.width * 0.08, dy: bounds.height * 0.05)
            ...
            drawString(NSAttributedString(string: started ? "\(score) out of \(maxScore), \(scoreClassificationName)" : "Not played", attributes: [NSFontAttributeName: UIFont.systemFont(ofSize: mainRect.height * 0.3, weight: UIFontWeightSemibold), NSForegroundColorAttributeName: UIColor.white, NSParagraphStyleAttributeName: para]), inRect: mainRect)
        }else {
            ...
            let font = UIFont.systemFont(ofSize: top * 0.75, weight: UIFontWeightRegular)
            
            if !started {
                drawString(NSAttributedString(string: "0%", attributes: [NSFontAttributeName: font, NSForegroundColorAttributeName: bottomColor]), inRect: CGRect(x: 0, y: bounds.height - top, width: bounds.width, height: top))
            }else {
                UIColor.black.setStroke()
                ...
                for(offset: i, element: (color: color, number: number)) in drawInfo.enumerated() {
                    ...
                    drawString(NSAttributedString(string: "\(Int(nearbyint(ratio * 100)))%", attributes: [NSFontAttributeName: font, NSForegroundColorAttributeName: UIColor.black]), inRect: CGRect(x: textX, y: textY, width: barX + barLength - textX, height: top), alignment: .right)
