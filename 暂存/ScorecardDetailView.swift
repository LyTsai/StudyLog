...
//        rightLabel.textColor = autoPlay ? UIColorGray(155) : UIColor.black
        cardsView.showsVerticalScrollIndicator = false
        addSubview(cardsView)
    }
    
    func setupWithMeasurement(_ measurement: MeasurementObjModel) {
        let riskKey = measurement.riskKey!
        let cardsInfo = MatchedCardsDisplayModel.getScoreRankingCardsInMeasurement(measurement, highToLow: true)
...
//        cardsView.frame = CGRect(x: 0, y: autoPlaySwitch.frame.maxY + gap, width: bounds.width, height: bounds.height - titleLabel.frame.height - gap)
        ...
        cardsView.frame = CGRect(x: 0, y: collectionY, width: bounds.width, height: bounds.height - collectionY)
    }
