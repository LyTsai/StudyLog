...
class AgeInputView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        bubblePath.addArc(withCenter: CGPoint(x: leftUp.x, y: mainRect.midY), radius: radius, startAngle: CGFloat(Double.pi / 2), endAngle: 3 * CGFloat(Double.pi / 2), clockwise: true)
        bubblePath.addLine(to: rightUp)
        bubblePath.addArc(withCenter: CGPoint(x: rightUp.x, y: mainRect.midY), radius: radius, startAngle: -CGFloat(Double.pi / 2), endAngle: CGFloat(Double.pi / 2), clockwise: true)
        bubblePath.addLine(to: CGPoint(x: thumbX + arrowHeigth * 0.8, y: mainRect.maxY))
        bubblePath.close()
