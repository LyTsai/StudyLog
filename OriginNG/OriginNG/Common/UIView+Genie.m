                          ...
                          completion:(void (^)())completion
{
    ...
    [slices enumerateObjectsUsingBlock:^(CALayer *layer, NSUInteger idx, BOOL *stop) {
        ...
        // opacity
        CABasicAnimation* showAnim = [CABasicAnimation animation];
        showAnim.keyPath = @"opacity";
        if (reverse == TRUE)
        {
            showAnim.fromValue = [NSNumber numberWithFloat:.0];
            showAnim.toValue = [NSNumber numberWithFloat:1.0];
        }else
        {
            showAnim.fromValue = [NSNumber numberWithFloat:1.0];
            showAnim.toValue = [NSNumber numberWithFloat:.0];
        }
        showAnim.duration = duration;
        showAnim.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseOut];
        showAnim.removedOnCompletion = NO;
        showAnim.fillMode = kCAFillModeForwards;
        [layer addAnimation:showAnim forKey:@"opacity"];
        // end opacity
    }];
