...
//    var cardInfoKey: String!
    ...
    func changeAnswerToIndex(_ answerIndex: Int) {
        ...
        if cardStyleKey == SetOfCardsCardTemplateView.styleKey() {
            option = cardInfo.cardOptions[answerIndex]
            value = answerIndex
        }
        
        CardSelectionResults.cachedCardProcessingResults.addUserCardInput(UserCenter.sharedCenter.currentGameTargetUser, metricKey: metricKey, cardKey: cardInfo.key, riskKey: RiskMetricCardsCursor.sharedCursor.focusingRiskKey, selection: option, value: value as NSNumber?)
    ...
    class func getMatchedCardOfUser(_ userKey: String) -> [MatchedCardsDisplayModel] {
        ...
        if let allMeasurements = CardSelectionResults.cachedCardProcessingResults.getAllUserRiskMeasurementsByCard()[userKey] {
            ...
            for (measurementValue, values) in sortedValues {
                ...
                matchedCard.categoryKey = matchedCard.cardInfo.metricGroupKey ?? "Others"
