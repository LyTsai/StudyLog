...
class FaceView: UIView {
    ...
    @IBInspectable
    var eyeBrowTile: Double = 0.0 { didSet{setNeedsDisplay() } }//-1 full furrow, 1 full relax
    @IBInspectable
    ...
    private func pathForBrow(eye: Eye) -> UIBezierPath {
        ...
        let eyeRadius = skullRadius / Ratios.SkullRadiusToEyeRadius
        let tileOffset = CGFloat(max(-1, min(tile, 1))) * eyeRadius / 2
        let browStart = CGPoint(x: browCenter.x - eyeRadius, y: browCenter.y - tileOffset)
        let browEnd = CGPoint(x: browCenter.x + eyeRadius, y: browCenter.y + tileOffset)
        let path = UIBezierPath()
