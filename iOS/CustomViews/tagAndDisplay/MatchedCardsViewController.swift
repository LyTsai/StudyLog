...
class MatchedCardsViewController: MultipleUserViewController {
    ...
    override func viewDidLoad() {
        ...
        // cards
        let mainFrame = CGRect(x: 0, y: 64, width: width, height: height - 64 - 49)
    ...
    fileprivate func setupUIWithMatchedCards(_ matchedCards: [String: [MatchedCardsDisplayModel]]) {
        ...
        // size
        let mainFrame = CGRect(x: 0, y: 64, width: width, height: height - 64 - 49)
        let xMargin = 64 * width / 375
        let yMargin = 59 * height / 667
        let topHeight = 54 * height / 667
        let bottomHeight = 42 * height / 667
        
        // tags
        keys = Array(matchedCards.keys)
        var texts = [String]()
        for key in keys {
            texts.append(AIDMetricCardsCollection.standardCollection.getMetricGroupByKey(key)?.name ?? "Others")
        }
