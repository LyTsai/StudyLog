...
class TalkBubbleView: UIView {
    ...
    func setupTopBubbleWithX(_ bubblePoint: CGPoint, edgeInsets: UIEdgeInsets, confine: CGRect, radius: CGFloat, onTop: Bool) {
        // calculte
        ...
        self.frame = onTop ? UIScreen.main.bounds : CGRect(x: confine.minX, y: bubblePoint.y, width: confine.width, height: bubbleH + confine.minY - bubblePoint.y)
        

        let mainFrame = onTop ? CGRect(x: confine.minX, y: confine.minY, width: confine.width, height: bubbleH) : CGRect(x: 0, y: bounds.height - bubbleH, width: bounds.width, height: bubbleH)
        
        let bubblePath = UIBezierPath.getTopArrowBubblePathWithMainFrame(mainFrame, arrowPoint: onTop ? bubblePoint : CGPoint(x: bubblePoint.x - confine.minX, y: 0), radius: radius)
