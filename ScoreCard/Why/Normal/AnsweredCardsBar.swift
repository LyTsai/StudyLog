...
class AnsweredCardsBar: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if !showPercent {
            let textH = top * 0.6
            let topFont = UIFont.systemFont(ofSize: textH * 0.85, weight: UIFont.Weight.medium)
            let textW = barRect.width * 0.5
            drawString(NSAttributedString(string: drawInfo.count < 3 ? "Higher" : "Highest", attributes: [NSAttributedStringKey.font: topFont]), inRect: CGRect(x: fontFactor, y: 0 ,width: textW, height: textH), alignment: .left)
            drawString(NSAttributedString(string: drawInfo.count < 3 ? "Lower" : "Lowest", attributes: [NSAttributedStringKey.font: topFont]), inRect: CGRect(x: barRect.maxX - textW - fontFactor, y: 0, width: textW, height: textH), alignment: .right)
        ...
        for (offset: i, element: (number: number, color: color)) in drawInfo.enumerated() {
            ...
            if showPercent {
                ...
                drawString(NSAttributedString(string: "\(Int(nearbyint(ratio * 100)))%", attributes: [.foregroundColor: UIColorFromRGB(88, green: 89, blue: 91), .font: UIFont.systemFont(ofSize: rectRemained.height * 0.8, weight: .medium)]), inRect: rectRemained, alignment: .right)
            }
        }
        ...
        if focusIndex != nil {
            ...
            let resultS = NSAttributedString(string: resultText, attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: bottomTextH * 0.8, weight: UIFont.Weight.medium)])
