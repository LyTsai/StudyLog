...
class GetBrainAgeView: UIView {
    ...
    fileprivate var displayAge: Int! {
        if let dobString = userCenter.getDobStringOfUser(userKey) {
            let age = CalendarCenter.getAgeFromDateOfBirthString(dobString)
    ...
    fileprivate func setupFrames() {
        ...
        titleLabel.font = UIFont.systemFont(ofSize: 12 * oneW, weight: UIFont.Weight.semibold)
        ...
        for (i, image) in brainImages.enumerated() {
            ...
            labels[i].font = UIFont.systemFont(ofSize: 12 * oneW, weight: UIFont.Weight.semibold)
        }
        
        pickButton.frame = CGRect(center: CGPoint(x: bounds.midX, y: bounds.height * 0.83), width: 182 * oneW, height: 35 * oneW)
        pickButton.titleLabel?.font = UIFont.systemFont(ofSize: 14 * oneW, weight: UIFont.Weight.semibold)
    ...
    func ageIsChanged() {
        ...
        let ageString = NSMutableAttributedString(string: "\(displayAge!)", attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: pickButton.frame.height * 22 / 35, weight: UIFont.Weight.bold)])
        ageString.append(NSAttributedString(string: " years old", attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: pickButton.frame.height * 14 / 35, weight: UIFont.Weight.bold)]))
