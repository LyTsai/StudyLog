...
//    var scoreStyle = CardViewBackStyle.iiaBack
    ...
    @IBOutlet weak var letterMailBox: UIImageView!
    // outlets
...
//    fileprivate var forOpen = true
    ...
    fileprivate func adjustUI() {
        ...
        let gColor = collection.getRiskTypeByKey(riskTypeKey)?.realColor ?? UIColorFromRGB(66, green: 147, blue: 33)
        
        gradientLayer.colors = [gColor.withAlphaComponent(0.6).cgColor, gColor.cgColor]
        topGradient.layer.insertSublayer(gradientLayer, at: 0)
        
        
        // state
        backView.transform = CGAffineTransform(translationX: 0, y: height * 0.5).scaledBy(x: 0.7, y: 0.7)
        letterView.transform = CGAffineTransform(scaleX: 0.01, y: 0.01)
    ...
    fileprivate func layScoreCard() {
        ...
        // init state for panel
        let angle = CGFloat(Double.pi) * (ratio - 1)
        
        let radius = dialChartView.radius * 1.3
        ...
        panelPointer.layer.position = CGPoint(x: dialChartView.frame.midX, y: dialChartView.frame.maxY)
    ...
    fileprivate func fillData() {
        ...
        dialChartView.segNumberAsPercent = (riskTypeKey != iPaKey)
        ...
        for (key, cards) in iden {
            if let classification = collection.getClassificationByKey(key) {
                ...
                if isThree {
                    tempPanel.append((cards: cards, color: classification.realColor ?? UIColor.lightGray, cards.first!.currentMatch()?.seqNumber ?? 0))
                }
            }else {
                ...
                if isThree {
                    ...
                    if cards.first?.cardOptions.count == 1 {
                        ...
                        // MARK: ------ the seqNumber of iKa is nil now
                        tempPanel.append((cards, supposed, cards.count))
                    }
                }
            }
        }
        ...
        let scoreNumber = MatchedCardsDisplayModel.getTotalScoreOfRisk(risk.key)
        ...
        bottomTitleLabel.text = "Matched Card Assessment Distribution( Assessment Adjudicator: \(risk.author?.name ?? "AnnielyticX"))"
    }
    ...
    func showLetter() {
        if !letterOnShow {
            ...
            UIView.animate(withDuration: 0.6, animations: {
                self.openHintLabel.transform = CGAffineTransform(translationX: 0, y: height * 0.5)
                self.mailBox.transform = CGAffineTransform(translationX: 0, y: -height * 0.24).scaledBy(x: 0.65, y: 0.65)
    ...
    fileprivate func openLetter() {
        UIView.transition(with: letterImageView, duration: 0.4, options: .transitionFlipFromLeft, animations: {
            ...
            self.openHintLabel.isHidden = true
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        // panel is touched
        if !letterOnShow && riskTypeKey != iPaKey && riskTypeKey != iCaKey {
    ...
    @IBAction func checkDetail(_ sender: UIButton) {
        ...
        UIView.animate(withDuration: 0.6, animations: {
            self.detailView.transform = self.forDetail ? .identity : CGAffineTransform(translationX: width, y: 0)
