...
class SimpleNode: UIView {
    var isDisabled = false {
        didSet{
            circleColor = isDisabled ? UIColorGray(200) : UIColor.colorFromRGB(126, green: 211, blue: 33)
        }
    }
    ...
    var circleColor = UIColor.colorFromRGB(126, green: 211, blue: 33)
    ...
    var imageUrl: URL! {
        didSet{
            if imageUrl != nil {
                ...
                image = ProjectImages.sharedImage.indicator
                ...
                imageView.sd_setImage(with: imageUrl, placeholderImage: ProjectImages.sharedImage.indicator, options: .refreshCached, progress: nil) { (image, error, type, url) in
                    if image == nil {
                        do {
                            self.image = UIImage(data: try Data(contentsOf: self.imageUrl, options: []))
                        }catch {
    ...
    override func draw(_ rect: CGRect) {
        ...
        // draw text
        if text != nil {
            font = UIFont.systemFont(ofSize: boldFont ? 16 * fontFactor: 9 * fontFactor, weight: boldFont ? UIFontWeightBold : UIFontWeightSemibold)
            let textStyle = NSMutableParagraphStyle()
            textStyle.lineBreakMode = .byWordWrapping
            textStyle.alignment = .center
            attributedString = NSAttributedString(string: text, attributes: [NSFontAttributeName: font, NSParagraphStyleAttributeName: textStyle])
            if image == nil {
        ...
        if tagged {
            ...
            //            let tagL = length * 0.5
            //            tag.move(to: CGPoint(x: bounds.midX - tagL * 0.5, y: bounds.midY))
            //            tag.addLine(to: CGPoint(x: bounds.midX + tagL * 0.5, y: bounds.midY))
            //            tag.move(to: CGPoint(x: bounds.midX, y: bounds.midY - tagL * 0.5))
            //            tag.addLine(to: CGPoint(x: bounds.midX, y: bounds.midY + tagL * 0.5))
            //
            //            UIColor.black.withAlphaComponent(0.8).setStroke()
            //            tag.lineWidth = lineWidth * 3
            //            tag.stroke()
        }
    }
}
...
class ButterflyNode: SimpleNode {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if useStandard {
            ...
            font = UIFont.systemFont(ofSize: boldFont ? 18 * standP: 9 * standP, weight: boldFont ? UIFontWeightBold : UIFontWeightSemibold)
        }
