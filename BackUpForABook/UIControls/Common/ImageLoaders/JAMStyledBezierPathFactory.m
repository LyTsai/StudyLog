...
- (NSNumber *)fillOpacityForKey:(NSString *)key;
- (NSNumber *)strokeOpacityForKey:(NSString *)key;
...
-(void)saveImage:(NSDictionary *)attributes;
{
...
- (JAMStyledBezierPath *)createStyledPath:(UIBezierPath *)path withAttributes:(NSDictionary *)attributes;
{
    return [self styledPathWithBezierPath:path
                              ...
                              fillOpacity:[attributes opacityForKey:@"fill-opacity"]
                              strokeColor:[attributes strokeColorForKey:@"stroke"]
                            strokeOpacity:[attributes opacityForKey:@"stroke-opacity"]
...
- (JAMStyledBezierPath *)styledPathWithBezierPath:(UIBezierPath *)bezierPath
                                      ...
                                      fillOpacity:(NSNumber *)fillOpacity
                                      strokeColor:(UIColor *)strokeColor
                                    strokeOpacity:(NSNumber *) strokeOpacity
                                        ...
                                        transform:(NSValue *)transform;
{
    ...
    float alph = opacity ? opacity.floatValue : 1.0;
    float fillAlph = fillOpacity ? fillOpacity.floatValue * alph : alph;
    float strokeAlph = strokeOpacity ? strokeOpacity.floatValue * alph : alph;
    
    // there are three cases with "fill" attaributes:
    // (1) non specified
    // (2) fill = hex value
    // (3) fill = url
    // case (1) use 0x000000 for fill color
    // case (2) use specified color
    // case (3) use the drawing pointed by the url
    styledBezierPath.gradient = [self gradientForFillURL:url];
    if (styledBezierPath.gradient == nil)
    {
        styledBezierPath.fillColor = fillColor;
    }
    
    styledBezierPath.fillOpacity = [[NSNumber alloc] initWithFloat:fillAlph];
    styledBezierPath.strokeOpacity = [[NSNumber alloc] initWithFloat:strokeAlph];
