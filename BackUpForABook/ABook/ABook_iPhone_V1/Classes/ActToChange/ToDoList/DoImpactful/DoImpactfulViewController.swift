...
class DoImpactfulViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let titleLabel = UILabel(frame: CGRect(x: 0, y: 64, width: width, height: 48 * height / 667))
        ...
        collection = PlansCollectionView.createWithFrame(CGRect(x: 0, y: titleLabel.frame.maxY, width: width, height: height - titleLabel.frame.maxY - 49), plans: plans!, date: date)
        collection.hostVCDelegate = self
        view.addSubview(collection)
        
        let addHeight = 64 * height / 667
        addView = UIView(frame: CGRect(x: 0, y: height - 49 - addHeight, width: width, height: addHeight))
        addView.backgroundColor = UIColor.white.withAlphaComponent(0.8)
        view.addSubview(addView)
        addView.isHidden = true
        
        let addButton = UIButton.customNormalButton("Add plans")
        addButton.adjustNormalButton(CGRect(center: CGPoint(x: addView.frame.midX,y: addHeight * 0.55), width: 168, height: addHeight * 0.8))
