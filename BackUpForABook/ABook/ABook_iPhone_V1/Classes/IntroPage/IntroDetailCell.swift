...
class MoreView: UIView {
    ...
    override func layoutSubviews() {
        ...
        label.frame = CGRect(x: bounds.width - rightMargin * 2.8, y: bounds.height - sub * 3, width: rightMargin * 1.8 - 3 * sub, height: sub * 3)
        arrow.frame = CGRect(x: label.frame.maxX + sub, y: label.frame.midY, width: 2 * sub, height: sub)
        label.font = UIFont.systemFont(ofSize: sub * 2.2)
    }
}
