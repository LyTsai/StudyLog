...
class PlayForOthersView: UIView, UISearchBarDelegate {
    ...
    fileprivate func setupViewWithFrame(_ frame: CGRect) {
        ...
        let tested = CardSelectionResults.cachedCardProcessingResults.gamePlayed(loginUser.key)
        ...
        */
        ...
        // table
        othersTable = OtherUsersTableView.createWithFrame(CGRect(x: gap, y: firstLineY, width: bounds.width - 2 * gap, height: 0.5 * bounds.height))
        ...
        addButton.frame = CGRect(x: bounds.midX - buttonWidth * 0.5, y: topMargin + othersTable.frame.maxY, width: buttonWidth, height: buttonHeight)
        addButton.setBackgroundImage(UIImage(named: "button_greenBG"), for: .normal)
        addButton.setTitle("Add people", for: .normal)
        addButton.titleLabel?.textColor = UIColor.white
        addButton.titleLabel?.font = UIFont.boldSystemFont(ofSize: buttonHeight / 3.06)
