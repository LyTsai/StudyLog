...
class UserDisplayView: UIView {
    ...
    func fillUserInfoData(_ userInfo: UserinfoObjModel)  {
        userIcon.image = convertDataObjectToImage(userInfo.imageObj) ?? UIImage(named: "userIcon")
        let name = userInfo.name ?? "Not Given"
        let age = CalendarModel.ageForDateOfBirth(userInfo.dob)
