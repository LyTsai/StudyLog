...
class TwoLevelsGraphView: UIView {
    ...
    func resizeCellsWithSize(_ size: CGSize) {
        ...
        let angleGap = Calculation().angleGap(surroundings.count, totalAngle: 2 * CGFloat(M_PI))
        for (i, sub) in surroundings.enumerated() {
            let angle = Calculation().getAngle(i, totalNumber: surroundings.count, startAngle: 0, endAngle: 2 * CGFloat(M_PI))
