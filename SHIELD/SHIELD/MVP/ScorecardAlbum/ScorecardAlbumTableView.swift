...
class ScorecardAlbumTableView: UITableView, UITableViewDataSource, UITableViewDelegate {
    ...
    var reverse = false {
        didSet{
            if reverse != oldValue {
                switch sortRule {
                case .gameSubject: subjectTableData.reverse()
                case .gameType: typeTableData.reverse()
                case .date: dateTableData.reverse()
                }
                reloadData()
            }
        }
    ...
    fileprivate func setDateTableData() {
        ...
        for key in risks {
            if let _ = collection.getRisk(key) {
                ...
                if let m = selectionResults.getLastMeasurementOfUser(userCenter.currentGameTargetUser.Key(), riskKey: key, whatIf: WHATIF) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let measurement = selectionResults.getLastMeasurementOfUser(userCenter.currentGameTargetUser.Key(), riskKey: risk, whatIf: forWhatIf)
        selectionResults.useMeasurementForDisplay(measurement)
        cell = ScorecardAlbumCell.cellWithTableView(tableView, riskKey: risk, measurement: measurement, sortRule: sortRule, isLast: indexPath.item == numberOfRows(inSection: indexPath.section) - 1, row: indexPath.row)
