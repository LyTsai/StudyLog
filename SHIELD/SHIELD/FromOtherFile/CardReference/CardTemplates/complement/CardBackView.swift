...
class CardBackView: UIView {
    ...
    var chosenColor = UIColorFromRGB(242, green: 234, blue: 181)
    var chosenRimColor = UIColorFromRGB(139, green: 87, blue: 42)
    ...
    var mainCornerRadius: CGFloat = 8
    ...
    override func draw(_ rect: CGRect) {
        ...
        let rimPath = path.copy() as! UIBezierPath
        
        path.append(UIBezierPath(roundedRect: bounds, cornerRadius: mainCornerRadius))
        path.usesEvenOddFillRule = true
        (isChosen ? chosenColor : UIColor.white).setFill()
        path.fill()
        
        // rim line
        (isChosen ? chosenRimColor : mainColor).setStroke()
        rimPath.lineWidth = rimLineWidth
        rimPath.stroke()
    }
}
