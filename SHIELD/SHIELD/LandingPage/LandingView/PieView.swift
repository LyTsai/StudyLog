...
@objc protocol PieViewDelegate {
    @objc optional func pieView(_ pie: UIView, didMoveTo index: Int)
    @objc optional func pieView(_ pie: UIView, isRotatingTo index: Int)
}
...
//            button.frame = Calculation.inscribeSqureRect(angleGap, startAngle: startAngle, radius: innerRadius, vertex: viewCenter)
            ...
            let bottomR = innerRadius * 0.46
            let imageL = bottomR * 2 * sin(angleGap * 0.5)
            let maxH = innerRadius * cos(angleGap * 0.5)
            let bottomH = imageL * 0.5 / tan(angleGap * 0.5)
            let bWidth = 2 * (imageL + bottomH) * tan(angleGap * 0.5)
            let bHeight = maxH - bottomH
            
            let bCenter = Calculation.getPositionByAngle(startAngle + angleGap * 0.5, radius: maxH - 0.5 * bHeight, origin: viewCenter)
            button.frame = CGRect(center: bCenter, width: bWidth, height: bHeight)
            
            button.fontRatio = 0.14
            button.verticalWithText(" ", image: nil, heightRatio: 1 - imageL / bHeight)
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if pieDelegate != nil {
            pieDelegate.pieView?(self, isRotatingTo: selectedIndex)
        }
    }
    ...
    func scrollToAngle() {
        ...
        if currentTop == selectedIndex {
            ...
            if self.pieDelegate != nil {
                pieDelegate.pieView?(self, didMoveTo: selectedIndex)
            }
        }else {
            ...
            UIView.animate(withDuration: min(0.15 * Double(indexGap), 1), delay: 0, options: .curveEaseInOut, animations: {
                ...
                self.adjustAngle()
                if self.pieDelegate != nil {
                    self.pieDelegate.pieView?(self, didMoveTo: self.selectedIndex)
                }
            }
        }
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: gap * 0.66, weight: .bold), .foregroundColor: UIColor.white]
        let decoAttributes = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: gap * 0.66, weight: .bold), .strokeColor: UIColor.black, .strokeWidth: NSNumber(value: 15)]
