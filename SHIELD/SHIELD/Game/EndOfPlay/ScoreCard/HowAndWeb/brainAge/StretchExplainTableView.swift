...
class StretchExplainTableView: UITableView, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        // height
        if onShow {
            let cH = NSAttributedString(string: texts[indexPath.section], attributes: [ .font: UIFont.systemFont(ofSize: 12 * bounds.width / 315, weight: .medium)]).boundingRect(with: CGSize(width: bounds.width - bounds.width / 315 * 20, height: height), options: .usesLineFragmentOrigin, context: nil).height + 15 * bounds.width / 315
            return cH
        }
